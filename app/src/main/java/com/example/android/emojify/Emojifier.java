...
class Emojifier {
    ...
    private static final float EMOJI_SCALE_FACTOR = 1.5f;
    ...
    static Bitmap detectFacesAndOverlayEmoji(Context context, Bitmap picture) {
        ...
        // OK (3): Change the name of the detectFaces() method to detectFacesAndOverlayEmoji() and the return type from void to Bitmap
        ...
        // OK (7): Create a variable called resultBitmap and initialize it to the original picture bitmap passed into the detectFacesAndOverlayEmoji() method
        ...
        if(faces.size() == 0) {
            ...
            for (int i = 0; i < faces.size(); ++i) {
                ...
                // OK (4): Create a variable called emojiBitmap to hold the appropriate Emoji bitmap and remove the call to whichEmoji()
                // OK (5): Create a switch statement on the result of the whichEmoji() call, and assign the proper emoji bitmap to the variable you created
                // OK (8): Call addBitmapToFace(), passing in the resultBitmap, the emojiBitmap and the Face  object, and assigning the result to resultBitmap

                //increment emojis in faces
    ...
    private static Emoji whichEmoji(Face face) {
        ...
        // OK (1): Change the return type of the whichEmoji() method from void to Emoji.
        ...
        // OK (2): Have the method return the selected Emoji type.
        return emoji;
    }

    // OK (6) Create a method called addBitmapToFace() which takes the background bitmap, the Emoji bitmap, and a Face object as arguments and returns the combined bitmap with the Emoji over the face.
    private static Bitmap addBitmapToFace(Bitmap backgroundBitmap, Bitmap emojiBitmap, Face face) {

        //create de bitmap mutable copy
        Bitmap resultBitmap = Bitmap.createBitmap(backgroundBitmap.getWidth(), backgroundBitmap.getHeight(), backgroundBitmap.getConfig());

        //dimens emoji to scale in face
        ...
        //set emoji size
        Bitmap scaledBitmap = Bitmap.createScaledBitmap(emojiBitmap, newEmojiWidth, newEmojiHeight, false);
